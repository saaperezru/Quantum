...
def createBasisViewer(basisDirectory,htmlPath,Reducer):
  ...
  return  Control.imageViewGenerator(Reducer.reduction.basisM.max(),Reducer.reduction.basisM.min(),basisDirectory,htmlPath,112,True)
...
def createReconstructionViewer(imagesDirectory,htmlPath,Reducer):
  try:
    ...
    print "Error Creating folder"
  return  Control.imageViewGenerator(Reducer.reduction.reconstructed.max(),Reducer.reduction.reconstructed.min(),imagesDirectory,htmlPath,112,True)
...
def createOriginalViewer(imagesDirectory,htmlPath,Reducer):
  try:
    ...
    print "Error Creating folder"
  return  Control.imageOriginalViewGenerator(255,imagesDirectory,htmlPath,112,True)

def generateFactorization(directoryName,reductionMethod,M,Docs,DocsP,p,r):
...
generateFactorization("QLSA2",Control.QLSA2,M,Docs,DocsP,p,r)
